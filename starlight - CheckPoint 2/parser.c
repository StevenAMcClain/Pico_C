...
PRIVATE volatile uint32_t parser_engine_idx;
...
PRIVATE void do_blob_trigger(TRIG_ID trig_id)
//
// Immediately Start playing a BLOB on the trigger list.
{
    Blob_Run(parser_engine_idx, Get_Trigger_Prog(trig_id));
}
...
//------
        ...
        case MATCH_LOAD_WAIT: // BLOB: Load a new binary object containing program and start running.
        {
            int num = read_num();

            if (BPage_Load_Blob(num)) { BTPRINTF("Page %d loaded and ready.\n", num); }
            else                      { BTPRINTF("Page %d can't load!\n", num); }
            break;
        }
        case MATCH_SAVE:
        {
            ...
            if (BPage_Save_Blob(num)) { BTPRINTF("Blob save to page %d of flash.\n", num); }
        ...
        case MATCH_ERASE:
        {
            ...
            if (BPAGE_VALID(bpage))
            {
                if (BPage_Erase_Page(bpage)) { BTPRINTF("Flash page %d erased.\n", bpage); }
...
//------
        case MATCH_SPHY: // Set the current led array to use.
        {
            ...
            Beng_Set_Phy_mask(Engine_Idx, phy_mask);
            BTPRINTF("SPHY %d\n", phy_mask);
            break;
        }
//------
