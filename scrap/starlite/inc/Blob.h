...
#define MAX_BLOB_SIZE (64 * 1024)    // Fairly arbitrary max size
...
extern uint8_t* Get_New_Blob_Base();
extern void Set_New_Blob_Base();

extern void Blob_NumLeds(int n);                    // Set number of LEDS.   Call when Num_Leds changes.
extern bool Blob_Load(uint8_t* /* blob_base */, size_t /* blob_size */, uint32_t /* check */);    // Call to load a new blob_base.
