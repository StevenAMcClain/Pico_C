...
PRIVATE void do_scale_led_data(uint32_t update)
{
    LEDS_PHY* phy = LEDS_Phy;
    uint32_t mask = 1;
    int i = 0;

    while (update && i < MAX_PHY)
    {
        if (mask & update)
        {
...
PUBLIC uint32_t* LED_Get_Scaled_LED_Data(int phy_idx)
{
    if (phy_idx < MAX_PHY)
    {
        return (uint32_t*)LEDS_Phy[phy_idx].scaled_led_data;
    }
    return NULL;
}
...
{
    do_scale_led_data(Needs_Update_Mask);
    WS2812_Do_Send(Needs_Update_Mask);   // Trigger DMA to start.... actually send the data.
    Needs_Update_Mask = 0;
}
