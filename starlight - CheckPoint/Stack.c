...
PUBLIC STACK* Stack_Initialize(void* buff, size_t size)         // Allocate a new queue.
{
	STACK *stack = (STACK*)buff;  // malloc(sizeof(STACK) + (sizeof(uint32_t) * size));

	if (stack)
	{
        memset(stack, 0, sizeof(sizeof(STACK) + (sizeof(uint32_t) * size)));
		stack->size = size;
		stack->buffer = AFTER_PTR(stack, sizeof(STACK));
