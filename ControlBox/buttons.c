...
//
PRIVATE uint8_t g_ui8ButtonStates = ALL_BUTTONS;
...
//
PUBLIC uint8_t Buttons_Poll(uint8_t *pui8Delta, uint8_t *pui8RawState)
{
    uint32_t ui32Delta;
    uint32_t ui32Data;
    ...
    if (pui8RawState) { *pui8RawState = (uint8_t)~ui32Data; }
    ...
    // caller.
    //
    if(pui8Delta) { *pui8Delta = (uint8_t)ui32Delta; }
...
//    MAP_SysCtlPeripheralEnable(BUTTONS_GPIO_PERIPH);
    ...
    //
    // Initialize the de-bounced button state with the current state read from
    // the GPIO bank.
    //
